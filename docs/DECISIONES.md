...
> **Prioridad vigente (Javier, 2026-09-12).** *"D1 y D2 esperan."* (D2 se resolvió el 2026-09-16.) Lo que manda
...
| D2 | ~~**Sacar el repo de OneDrive**~~ **RESUELTA el 2026-09-16** (miércoles, ~18:54, dato de Javier; coincide con la carpeta vieja, cuyo último commit es de ese día). El repo vive en `D:\dev\tropicana-app`, clonado limpio; el `.env.local` quedó a salvo en un gestor de contraseñas (confirmado por Javier al arrancar el Paso 2D). El stash que preocupaba en la carpeta vieja se inspeccionó el 2026-09-18: **estaba vacío** (mismo contenido que su commit base, 0 líneas de diferencia), no había nada que rescatar. Lo único que queda es manual y sin urgencia: eliminar la carpeta vieja de OneDrive, ya verificado que no tiene nada sin commitear ni sin pushear. La carpeta de handoff de Design **no se toca** (`scripts/sync-design.mjs`). Procedimiento y contexto en `docs/MUDANZA_REPO.md`. | **Resuelta** | — | — |
