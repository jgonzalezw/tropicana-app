...
| D17a | **Registrar quién dictó la clase, al tomar asistencia.** Mostrar el **titular vigente de esa fecha**; poder marcar que la dio un **reemplazante**, con **quién**, el **motivo** (catálogo) y **cuánto se le paga** (referencia: la tarifa del profesor). Si el curso está **desasignado** ese día, el reemplazo es **obligatorio** salvo que la clase se cancele. *El campo ya existe*: `sesiones.profesor_id` se escribe hoy **por inferencia** —se estampa la asignación vigente al guardar— y nadie lo lee. Esto lo convierte en un hecho registrado en vez de una suposición, que es lo que pide la regla 20. | **Pendiente — recomendado como lo próximo** (2026-09-12) | Apareció al corregir D18. | **Cuanto antes**: cada clase que se registre sin esto pierde para siempre quién la dictó de verdad, y no se puede reconstruir después. No depende de D12 — el catálogo (`catalogos` + `catalogo_valores`) ya existe y ya se usa. |
