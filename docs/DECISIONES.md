...
| D2 | **Sacar el repo de OneDrive** (`C:\Users\Javier\onedrive\natalia\tropicana-app` → **`D:\dev\tropicana-app`**, elegido por Javier el 2026-09-12). **Es la única cura**: se evaluó mandar la carpeta de build afuera con `distDir` y la doc de Next lo prohíbe — *"distDir should not leave your project directory"*. No hay arreglo por configuración. **El motivo por el que estaba en OneDrive era el respaldo ante falla de disco** (Javier, 2026-09-12), y ese motivo está cubierto: el código vive en GitHub y se restituye con un `git clone`. Lo que OneDrive sí cubría y git **no** es `.env.local` (ignorado por `.gitignore`, y con razón) y los `validacion_*.xlsx`. Antes de mover la carpeta, esas dos cosas necesitan su propio lugar — el `.env.local` en un gestor de contraseñas, no en otra carpeta. | **ACORDADO, sin fecha — Javier lo dejó para más adelante** (2026-09-12: *"desisto de hacerlo en este momento"*). Llegó a correr el **paso 1** y ahí quedó. | Se destrabó al confirmarse que GitHub cubre el respaldo que era el motivo de tenerlo en OneDrive, pero cortarlo ahora le interrumpía el día. **No preguntarle cuándo**: lo retoma él. Cuando lo haga, entregarle `docs/MUDANZA_REPO.md` y arrancar por el stash, que es lo único sin resolver. | **Ya costó tres veces** (`2a26010`, `8fb648c`, y el 404 de `/liquidaciones/2`). **El procedimiento completo, con los comandos, está en `docs/MUDANZA_REPO.md`** — y **hay que entregárselo a Javier cuando vaya a hacer la mudanza** (pedido explícito, 2026-09-12: *"me lo entregas cuando lo hagamos. No dejes de recordarlo"*). **Recordárselo aunque no pregunte.** Resumen:  (1) confirmar que la carpeta vieja no tiene nada sin commitear ni pushear; (2) guardar los valores de `.env.local` en un gestor de contraseñas —es lo único que OneDrive cubría y GitHub no—; (3) clonar limpio en `D:\dev\tropicana-app` en vez de arrastrar la vieja, así `.next` y `node_modules` quedan atrás; (4) copiar `.env.local`, `npm install`, `npm run dev:limpio`; (5) verificar que el aviso de `scripts/chequeo-entorno.mjs` ya no aparece. **La carpeta de handoff de Design NO se toca**: `scripts/sync-design.mjs` apunta a `C:\Users\Javier\OneDrive\Natalia\tropicana-Claude.Design-Handoff`, que es otra carpeta y tiene que seguir sincronizando — es por donde Design entrega. **Y la carpeta vieja no se borra desde OneDrive**: borrar ahí borra también la copia de la nube. Se la deja unos días y después se elimina. **HALLAZGO del 2026-09-12, sin resolver:** la carpeta vieja tiene un **stash** (`stash@{0}`, WIP sobre `0c1815d`, *Etapa 1 — Inscribir y cobrar*, del 01/09, **155 commits atrás**). Un stash **no viaja en un clon ni está en GitHub**: muere con la carpeta. Javier tenía que mirarlo con `git stash show --stat stash@{0}` y decidir: descartarlo (lo más probable, es de la etapa 1 y el código cambió entero) o rescatarlo a una rama con `git stash branch` y pushearla **antes** de borrar la carpeta vieja. |
...
| D21 | **El registro retroactivo que impacta una liquidación ya cerrada/pagada no debe ser un bloqueo duro para todos — debe ser confirmar/cancelar para quien tiene autoridad.** Hoy (regla de negocio 16, `src/lib/periodos.ts`) el congelador bloquea **a cualquiera** que toque una clase de la que depende una comisión ya pagada. Encontrado el 2026-09-17 con Heels 29/08: una inscripción retroactiva a agosto generó una liquidación ya cerrada, y al cargar la asistencia de esa clase el sistema avisaba que la liquidación se modificaría, dejaba grabar, pero el mensaje de error quedaba pegado en pantalla (bug de UI separado, corregido ese mismo día en `ClienteAsistencia.tsx`: `error` no se limpiaba al cambiar de curso/fecha, igual que ya pasaba con `aviso`). Al revisar el caso, Javier señaló que el problema de fondo no era el mensaje pegado sino que el bloqueo contradice la política ya definida para estos casos.<br><br>**Política decidida (Javier, 2026-09-17), a construir cuando se dispare:**<br>• **El escenario no es solo la inscripción retroactiva**: también puede originarse en un registro o una modificación retroactiva de asistencia, o en la suspensión retroactiva de una clase — las tres tocan el mismo conteo de prorrateo de la regla 16.<br>• **Usuario sin autoridad** (Profesor, Asistente): el bloqueo actual **se mantiene tal cual está** — no puede forzar el impacto sobre una liquidación cerrada. Lo único que cambia es el mensaje: debe decir que hay que avisar a un usuario con autoridad (Gerente, Administrador), no solo que la acción está bloqueada.<br>• **Usuario con autoridad** (Gerente, Administrador): puede hacerlo, pero no en silencio — se le muestra el contexto (qué liquidación se ve afectada, qué cambia) y se le pide **reconfirmar** en su propia sesión antes de proceder. El contexto es lo que le permite decidir con criterio, no una advertencia genérica.<br>• **Qué se reliquida** (respuesta 3 de Javier): **las dos cosas** — la comisión propia de la membresía nueva (retroactiva) que se devenga por primera vez, **y** el delta que surge de recalcular el prorrateo de la membresía ya pagada (regla 10: agregar una membresía no debería tocar lo ya repartido, pero si el conteo de clases del curso cambia, sí lo hace). *"Lo que se reliquida es lo que no se pagó a quien correspondía."*<br>• **Dónde entra el complemento** (Javier reconsideró en la misma conversación, después de que se le recordara cómo funciona `generarLiquidacion`): **al mes en que ocurrió la clase** (el período original), reabriendo esa liquidación como un **complemento con saldo** — no al mes en que se hace la reliquidación. *"Permite enmendar un error o un desvío de inmediato."* Falta **verificar y definir las opciones según la liquidación original ya haya sido pagada o no** (el complemento con saldo es claro cuando no se pagó nada más de esa liquidación; falta el diseño para cuando sí).<br>• **La liquidación original pagada no se reescribe** — sigue la regla 16 tal cual: se complementa, no se corrige lo ya cobrado.<br>• **Mitigación inmediata, ya decidida pero sin ejecutar**: reducir `asistencia_semanas_retro` (hoy en 2 semanas) al mínimo operable, para achicar la ventana de exposición mientras el mecanismo de confirmación no existe. **El valor puntual quedó sin decidir** (2026-09-17, Javier prefirió dejarlo en 2 por ahora y resolver el número más adelante en vez de bajarlo a ciegas). | **Pendiente — alta prioridad, con disparador explícito** (Javier, 2026-09-17) | Es una pieza más grande que un fix de UI: toca el congelador (regla 16), el motor de complementos (`generarLiquidacion`) y el modelo de permisos (autoridad Gerente/Administrador vs. Profesor/Asistente — encaja con `rol_visibilidad`, migración 0043, todavía en dev). Javier explícitamente prefirió anotarla y seguir con la prioridad operativa vigente. | **Después de completar el plan en curso de ventas de particulares, alquileres y talleres** (C3/2D y lo que quede del Paso 5). Javier: *"debes dejarla anotada y recordarla luego que completemos el plan en curso de ventas de particulares, alquileres y talleres."* Avisar aunque no pregunte, apenas ese plan cierre (regla de proceso 10). Mientras tanto, cada inscripción/asistencia/suspensión retroactiva sobre una liquidación ya pagada sigue bloqueada sin vía de excepción para Gerente/Administrador, y `asistencia_semanas_retro` sigue en 2 semanas sin bajar. |
