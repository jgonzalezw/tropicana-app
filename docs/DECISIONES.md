...
| D5 | ~~**Clasificar mejor el motivo del cobro** en el recibo: Membresía, Clase Particular, Clase de Prueba, Alquiler, Taller, Venta Producto, Ajuste.~~ **CERRADA para particulares el 2026-09-25** (C3 H2): `venderParticular` asienta `clase_particular` solo, sin pedirlo a mano, y Caja (`lineasPorCobrar`) clasifica esas cuotas pendientes en el bucket `particulares`, no `cuotas`. Falta todavía el lado alquiler (H7). | **Particulares cerrado; alquiler pendiente.** | Alquiler se termina en **H7**. |
...
| D11 | **Inscribir a los acompañantes de una prueba grupal, con su crédito.** Hoy, en una prueba grupal, solo el **titular** puede convertir y se le acredita **su parte** (opción b: lo pagado ÷ personas). La parte de los acompañantes **existe pero no se puede reclamar**: no tienen nombre en el sistema. Javier: *"lo lógico es que el resto queda para cuando se inscriban los otros en plazo… haciendo referencia a algún ID que se entregue al momento de la inscripción, o referir al titular aunque no se haya inscrito… pero debe ir descontando la cantidad de beneficiarios. Si cualquiera deja pasar la fecha, igual lo pierde."* Sigue siendo de la **prueba grupal**, un caso distinto al de C3: el v2 de particulares (25/09, §7.4) resuelve el suyo aparte — "los demás asistentes se registran uno a uno o no, según el plan", sin crédito individual ni asistencia por persona, y sin afectar la liquidación del profesor (hito **H1**, política de asistentes del plan — **construida** el 2026-09-25: `planes.registra_acompanantes`, sin crédito ni asistencia individual; D11 en sí sigue abierta, es el caso de la prueba grupal). | **Pendiente** (2026-09-11) | Los acompañantes no tienen identidad en el modelo; darles una es un cambio de modelo, no un ajuste. Javier: *"dejarlo en b por ahora sin la inscripción de los restantes"*. | Cuando **aparezca el primer caso real** (un acompañante que quiere inscribirse y reclamar su parte), o en el paso siguiente si se prioriza. **Cada prueba grupal que se venda mientras tanto es un crédito que alguien puede venir a reclamar y no vamos a poder darle.** |
...
| D9 | **La tarjeta de confirmación de la venta tiene que ser específica**: plan + monto + cursos + días + fecha de la primera clase + si es clase de prueba. Hoy dice poco y hubo que ir a la base para saber qué había quedado. | Pendiente para el resto de las entidades. **Caso particulares cerrado el 2026-09-25** (C3 H2): la tarjeta de `venderParticular` muestra plan, horas, profesor, primera clase (o cuántas más si es agenda fija) y dónde, leído de lo guardado — y además dos `AvisoWhatsapp` (al titular o su tutor, y al profesor), no solo texto. | Se arregló lo urgente (que mostrara **todas** las fechas y el plural). Lo demás es un rediseño del mensaje. | Junto con D4, que también es cómo se le muestra una entidad a la persona. D4 sigue abierta para el resto de las entidades. |
| D22 | **Comisión por referido de profesor.** **Ya estaba decidida en concepto el 2026-09-04** (RF-02.2/RF-07.2): la cobra el **profesor que REFIRIÓ** al alumno, no el titular, con el `pct_referido` de **su** asignación, sobre lo que ese alumno paga en el curso de otro profesor. El `pct_referido` se carga en cada asignación desde la 0005, pero **la liquidación no lo lee**: la regla nunca se construyó. Al abrir esta fila el 2026-09-24 se la planteó como "falta decidir si paga algo", y eso **contradecía** la decisión del 04/09 (regla de proceso 8: vale la decisión anterior mientras otra no la cambie). Corregido el 2026-09-24. **Lo que sí queda abierto** es la brecha que ya estaba anotada el 04/09: **(a)** a quién apunta el referente. La 0048 ya lo resolvió en el modelo: el referente es una `contacto_relaciones` tipo `referido_por` y puede ser cualquier contacto, profesor incluido. `alumnos.referido_por_alumno_id` quedó marcada OBSOLETA. Lo que falta es que la liquidación lea esa relación. **(b)** Qué `pct_referido` aplica si el profesor tiene varias asignaciones. **(c)** Sobre qué cobros se aplica: ¿la venta inicial o también cada renovación? | **Decidida en concepto (2026-09-04), sin construir.** Faltan (b) y (c), y que la liquidación la lea. | Resolver (a) exigía un modelo de contactos que recién existe desde la 0048. | Cuando se quiera empezar a pagar referidos, o con el primer caso real. La base ya existe: `contacto_relaciones` (0048) y `pct_referido` (0005). |
...
No son backlog: Javier ya decidió que **así se debe trabajar**. Entraron acá
para que no se perdieran hasta que el código las alcanzara, y se quedan porque
explican el porqué del modelo. **Casi todas ya están construidas y en
producción**: la columna *Estado* dice dónde está cada una (revisada completa
el 2026-09-24).

| # | Decisión | Estado |
| --- | --- | --- |
| **El orden del Paso 5: la disponibilidad antes que la venta** (C1→C5) | Javier, 2026-09-12, en el documento de instrucciones del Paso 2. **Reemplaza el orden acordado esa misma mañana** ("validación mínima de choque dentro de 2D", con *Vender servicio* como paso siguiente). El motivo es un reencuadre del modelo, no una preferencia: *"la unidad atómica no es «la venta con horario» sino «la reserva de una franja de sala». Vender un paquete crea un saldo de horas; reservar consume ese saldo ocupando la sala — son actos distintos que pueden ocurrir juntos o separados en el tiempo."* Construir la venta primero dejaría la reserva colgada de la venta, y habría que desacoplarla después. Orden: **C1** horario base → **C2** disponibilidad + reserva con lista textual (sin grilla) → **C3** venta apoyada en la disponibilidad → **C4** agenda visual (pasa por Design) → **C5** conflicto bloqueo-vs-agendado. | **VIGENTE desde 2026-09-12.** Al 2026-09-25: **C1 ✅**, **C2 ✅** (producción 2026-09-17), **C3 = lo siguiente** (C3-0a, contactos, ya en producción; C3-0b espera a C3), **C4** después de C3 y pasa por Design (`ROADMAP.md` R2), **C5 a medias**: lado cursos regulares en producción desde el 2026-09-16, falta el lado reservas (`ROADMAP.md` R1). **C3 en sí se redefinió el 2026-09-25** con las definiciones v2 de Natalia — ver la fila de abajo, "C3 — definiciones v2". |
| **Cómo se clasifica por qué está tomada la sala** | Javier (2026-09-12): el motivo se **elige de una lista** y el responsable o la aclaración van en una **glosa abierta** — *"clasificar los motivos en un campo de motivos, y aclararlo o poner el responsable en un campo glosa abierto"*. Los motivos que salen del **horario base** no se eligen a mano, y el horario base son **dos piezas distintas**: el **patrón** —la regla semanal de apertura, de donde sale "fuera de horario"— y las **excepciones por fecha** —feriados y cualquier otro cierre o apertura especial—. Un feriado **no** es un bloqueo que alguien carga a mano: es una excepción del horario. Además, una reserva lleva **notas**, y no son un memo: *"pueden ser incluso usadas como instrucciones o recomendaciones para el asistente coordinador de la sala"*, así que se muestran donde se opera la sala. **Vacío significa cerrado, no abierto**: un horario sin cargar deja la sala no reservable, porque el default contrario produce justo el bug que C1 viene a evitar. | **CONSTRUIDA y en producción.** Patrón + excepciones en C1 (0036/0037, producción 2026-09-16); glosa y notas de la reserva en C2 (0040, producción 2026-09-17). |
| **Una sala hoy, varias después** (ex D20) | Javier (2026-09-12): *"por el momento una sola sala, posteriormente podrían haber varias."* **Consecuencia de diseño, y es la parte que importa: se modela para N y se muestra para 1.** Existe la entidad `salas` desde el principio, con una sola fila; cada clase y cada bloqueo dicen en cuál están; y la pregunta de choque se hace **por sala** aunque hoy siempre sea la misma. La pantalla **no muestra el selector** mientras haya una sola —elegir entre una opción es ruido—, y aparece solo cuando se cargue la segunda. Agregar la dimensión después sería rehacer el modelo y remapear lo ya agendado, no agregar un campo. | **CONSTRUIDA y en producción** (0035 `salas`; 0037 la segunda sala, producción 2026-09-16). Con dos salas activas, las pantallas ya muestran todas o dejan elegir: la condición "una sola, sin selector" se cumplió y quedó atrás. |
| **La sala se puede bloquear sin venta** (ex D7) | **Sí**: la agenda de sala es un **calendario real**, no el reflejo de lo vendido. Javier (2026-09-12): *"la sala se puede reservar/bloquear sin paquete vendido. Aplica motivos de capacitaciones internas, preparación de coreografías de los profesores, mantenimiento, etc."* Consecuencias de diseño: un bloqueo **existe sin dueño comercial** —no cuelga de una venta ni de una membresía—, necesita **motivo de catálogo** (regla de negocio 13) y ocupa la sala igual que una clase para la validación de choque. | **CONSTRUIDA y en producción** desde el 2026-09-17 (C2: bloqueos con motivo de catálogo en `/sala`). |
| **Duración de la clase** (ex D6) | Un curso declara **cuánto dura una clase**, en el maestro de cursos. Javier (2026-09-12): *"implementá la duración de las sesiones de cursos en el maestro de cursos, con eso se obtiene la hora de fin."* **La hora de fin se calcula, no se guarda**: guardar inicio y fin sería tener el mismo hecho en dos campos que pueden contradecirse — la confusión más cara de este proyecto fue exactamente esa. | **CONSTRUIDA y en producción** desde el 2026-09-12 (migración 0034 + `src/lib/horarios.ts`). Los 9 cursos quedaron en 60 min, que es lo único que se podía suponer sin inventar: Javier corrige el que no sea. La lista fija de duraciones de la 0034 se reemplazó por múltiplos del intervalo estándar (0039, fila de abajo). |
...
| **Vigencia del curso** (ex D15) | Un curso tiene **fecha de activación y de baja**. El registro de asistencia es exigible **solo** entre esas fechas: fuera de ellas nunca se pide y nada queda "sin registrar". No se pueden vender planes ni pruebas con un curso inactivo — eso **inactiva el plan**. Si aparecen clases retroactivas, la fecha de activación **se corrige** (el usuario la ajusta antes de inscribir y la revalida): que haya clase retroactiva significa que sí había profesor, alumno y clase. Javier (2026-09-12): *"No lo veo como backlog. Es así como se debe trabajar."* Y el marco general: **todo lo retroactivo es una excepción de la operativa, no el día a día.** | **CONSTRUIDA y en producción** desde el 2026-09-12 (migración 0033 + `src/lib/vigencia.ts`). |
| **Intervalo estándar de tiempo** (ítem 3) | Un solo parámetro (`tiempos_incremento_min`, 30 min default / 1 hora) gobierna **todo** lo que se carga en minutos: duración de curso, duración mínima de una clase (`duracion_minima_curso_min`), horario de sala (patrón y excepciones). Javier (2026-09-16): *"usar siempre y como estándar el intervalo en minutos... Crear cualquier otro parámetro de tiempos de sala o duración de clases/sesiones o uso de paquetes que se requiera manteniendo el criterio de incrementos establecido."* **Cuando se construya C2/C3** (venta de clases particulares y alquiler de sala, hoy sin pantalla), sus horarios de reserva **tienen que reusar este mismo parámetro** — no crear uno nuevo ni asumir un paso propio. **Corrección del 2026-09-26 (Javier, al probar H3 en su local), solo para reservas**: la duración de una reserva y la hora en que puede empezar son **dos preguntas distintas**, y compartían una sola validación contra el incremento — eso rechazaba reprogramar una reserva vieja de 30 min (creada cuando el incremento todavía era 30) a una duración mayor perfectamente válida, con un mensaje que ni tenía sentido consigo mismo. *"La reserva no puede tener duración menor al mínimo del parámetro, solo se debe poder reservar para duraciones múltiplos del mínimo (regla general). El parámetro de los intervalos es para la hora de inicio."* Es decir: `duracion_minima_curso_min` gobierna la **duración** (múltiplos del mínimo), `tiempos_incremento_min` gobierna dónde puede **empezar** (alineada al incremento) — nunca la misma pregunta. | **CONSTRUIDA** en dev el 2026-09-17 (migración 0039 + `src/lib/horarios.ts`: `opcionesDuracion`/`esMultiploDe`). La duración de un curso pasó de texto libre a una lista de múltiplos del incremento; `duracion_clase_min` (0034) queda de baja, su lista fija {45,60,75,90,120} no era múltiplo de nada. **En producción desde el 2026-09-17.** **La corrección del 26/09 se construyó en dev el mismo día** (`validarTiempoReserva` + `opcionesDuracionReserva`/`horaAlineada` en `src/lib/horarios.ts`), aplicada a las reservas de particulares (H2 venta y H3) y a los bloqueos de `/sala`. **Alcance acotado a propósito**: la duración de un **curso** sigue con la regla vieja (múltiplo del incremento) — no hay reporte de que sea un problema ahí, y cambiarla sin que Javier lo pida violaría la regla de proceso 8. Sin pase a producción todavía (viaja con H3). |
...
| **C3 — definiciones v2 (particulares, alquiler, talleres)** | Relevamiento con Natalia el 23/09, revisado y cerrado con ella el 25/09. Reemplaza al del 23/09 —`docs/relevamientos/2026-09-25-C3-definiciones-v2.md`—. En una línea: **todo se vende por plan** (no hay venta sin plan; "venta directa" es solo el nombre del plan sencillo, sin cotización); **cinco tipos de servicio** — curso regular, particular, alquiler, taller y servicio especial (este último, con cotización, va en una etapa siguiente); **cinco criterios de liquidación** elegidos por el plan, los dos últimos solo para taller, con "período vencido" resuelto por el parámetro `periodicidad_liquidacion` que ya existe; **tres formas de pago al profesor**, elegidas en el plan — fee por hora (vive en el profesor), % sobre el margen (precio neto − costo de sala, según decida el plan) o monto fijo por membresía; una **membresía ya vendida se puede extender** dentro de los márgenes de su plan; **las sesiones de una membresía se reparten en varias salas**, propias (con validación) o externas (sin validación, con nombre descriptivo por membresía o, en talleres, por plan); **siete estados de reserva** — Solicitada, Confirmada, Reprogramada, Reagendar, Suspendida, Ausente, Realizada; y el **horario hábil de notificaciones** reusa la lógica de horario de sala (patrón + excepciones) sin reservar nada. **Precisiones de Javier el 25/09, al revisar el plan de construcción:** el **rol alumno se adquiere solo al comprar** — el titular de una membresía de curso regular, particular, taller o prueba pasa a ser alumno automáticamente (la venta crea la fila en `alumnos` si hace falta); en **alquiler no aplica**, el titular queda solo como contacto y no entra al padrón; en **servicios especiales** puede ir de las dos formas según quién compre. Una reserva **Solicitada ya ocupa** la sala y el profesor —*"no tiene sentido ver disponibilidad y tomar la decisión de actuar en base a un espacio disponible mientras coordino los demás recursos, y que cuando todo esté coordinado el espacio horario ya se asignó a otra persona"*—, con una validez máxima que la libera sola si nadie confirma (sin descontar saldo hasta confirmarse). Cancelar fuera de plazo deja la reserva en **Ausente**, con una marca de incumplimiento, no en un octavo estado. **El alquiler usa el mismo tratamiento de reservas que las particulares** (horas vendidas contra consumidas, los 7 estados, cierres por excepción y cancelaciones del cliente). **En talleres la reserva de sala es del plan**, no de cada membresía: se agenda al abrirlo a la venta y vale para todos los inscritos; si alguna sesión es afuera, se usa la sala externa genérica con su nombre en el plan. **Cada slot horario es una reserva independiente** *(Javier, 25/09)*: aunque se pidan varios juntos al planificar una membresía, cada uno se reagenda, suspende, cambia de sala o pasa a una sala externa por su cuenta; vale para particulares, alquileres y talleres. Las particulares descuentan **horas de clase** y los alquileres **horas de alquiler** de su membresía, según su tipo. **La validez de una Solicitada es de 24 horas**, como parámetro (`reserva_solicitud_validez_horas`). | Plan propuesto en nueve hitos (H1–H9), en `docs/relevamientos/2026-09-25-C3-plan-construccion.md`. **H1 y H2 construidos y validados en dev el 2026-09-25/26** (H1 por Javier a mano en su local; H2 con datos reales en dev, ver `ESTADO.md`), **H3 construido y verificado en dev el 2026-09-26** (los 7 estados, migración 0054), con el recorrido completo en el navegador contra datos reales, pero **todavía sin la prueba de Javier en su propio local** — los tres, **sin pase a producción todavía**, acumulados para un solo pase. Falta H4 cierres de sala sobre reservas (cierra el lado reservas de C5, `ROADMAP.md` R1/R22), H5 liquidación de particulares, H6 extensión de membresía, H7 alquiler, H8 talleres (pasa por Design, sin mockup), H9 horario hábil. Contraste completo contra el repo, con evidencia, en el mismo documento. **Reafirmación del 2026-09-26**: al probar H2, Javier pidió analizar una grilla semanal de slots (seleccionar día/hora visualmente contra la ocupación real de la sala) para reemplazar el "prueba y error" de elegir agenda a mano. Es el alcance de **C4**, no un agregado a H2/H3: planteado el trade-off, Javier confirmó **pausarlo y pedir el mockup a Design primero**, respetando la decisión del 2026-09-12 de abajo ("C4 pasa por Design"). Mientras tanto, la lista de "Revisar disponibilidad" de H2 (`ESTADO.md`) es el mecanismo interino. |
...
- **Rama activa:** `claude/adoring-maxwell-2car0j` — lleva H1 + H2 + H3
  (fast-forward sobre `claude/modest-hypatia-qymr45`, que traía H1+H2).
- **Último hito:** H3 (reservas con los 7 estados) — construido y verificado
  en dev (recorrido completo de los 7 estados en el navegador, tres bugs
  encontrados y corregidos en el camino). **Javier ya lo probó en su propio
  local** y reportó cinco problemas y dos pedidos de terminado (mensajes
  fuera del recuadro de la reserva, el paquete agotado sin bloquear el
  formulario, el botón de reprogramar mal ubicado, la regla de duración
  contra el incremento en vez del mínimo, y la falta de buscador/orden/volver
  en `/particulares`) — todos corregidos y reverificados en la misma sesión.
  **Sigue pendiente el OK explícito de Javier para el pase**, esta vez sobre
  la ronda 2. Detalle en `docs/ESTADO.md`, sección "C3 — H3: reservas con los
  7 estados" y su subsección "Ronda 2 — correcciones de la prueba de Javier
  en su local".
- **Sigue:** H4 (cierres de sala sobre reservas — el lado reservas de C5).
- **Para arrancar la sesión siguiente**, sobre la rama designada que
  corresponda:
  ```
  git fetch origin claude/adoring-maxwell-2car0j
  git merge --ff-only origin/claude/adoring-maxwell-2car0j
  ```
- **Para que Javier actualice su local** (regla de proceso 9, "Javier no
  programa: solo actualiza y prueba"):
  ```
  git fetch origin claude/adoring-maxwell-2car0j
  git pull origin claude/adoring-maxwell-2car0j
  npm run dev:limpio
  ```

## 4. Registro de pases a producción

**Hoy (2026-09-24) no hay nada pendiente de pase**: todo lo construido está en
producción, con migraciones 0001–0051 en las dos bases. Cuando algo quede
