...
| --- | --- | --- | --- | --- |
| D1 | **Unificar el nombre de la membresía en lo YA EXISTENTE.** Renombrar `inscripcion_id` → `membresia_id` en `asistencias`, `cuotas`, `pagos`, `corrimientos_ciclo` e `inscripcion_cursos`, y la tabla `inscripciones` → `membresias`. | **CONSTRUIDA en dev el 2026-09-24** (migración 0047). Falta el pase a producción. | — | Javier la adelantó al medir el informe de impacto de C3-0 (contactos): 11 de los 23 archivos que toca los iba a tocar C3-0a.1 de todos modos, así que ese código se escribe una sola vez. Evaluación completa y verificación en `docs/ESTADO.md`, bloque "D1 + D3". **El pase a producción** espera el OK de Javier y una ventana sin operación (el renombre no es aditivo). Control 15 ya da **OK** en dev. |
...
| D3 | **Renombrar `ClienteVentas.tsx`** a algo que diga lo que hace (`PestanasVenta` / `MostradorVenta`). El nombre indujo a Javier a creer que había otra pantalla de ventas. | **CONSTRUIDA en dev el 2026-09-24**, junto con D1. Falta el pase a producción. | — | — |
...
- **D1 + D3 (migración 0047) — CONSTRUIDA Y VERIFICADA EN DEV el 2026-09-24,
  sin pasar a producción.** Javier pidió pausar ahí (*"pausa al terminar d1 y
  d3 en dev"*): nada commiteado ni pusheado todavía. El pase necesita, además
  del OK explícito, una **ventana sin operación** (el renombre no es
  aditivo: el código publicado se cae contra el esquema renombrado unos 3-5
  minutos hasta que Vercel termina el deploy nuevo) y **un solo push**, como
  siempre. Detalle completo, con el bug de secuencias encontrado y corregido,
  en `docs/ESTADO.md`, bloque "D1 + D3 — un solo nombre para la membresía".
