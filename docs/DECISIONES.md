...
| **Alcance del congelador (regla 16): tomar asistencia de una clase congelada sí se permite** (ex D21) | Encontrado el 2026-09-17 con Heels 29/08: una inscripción retroactiva a agosto pedía sumar su alumno a la asistencia de una clase de la que ya dependía una comisión multi-curso pagada (de otro alumno), y el congelador bloqueaba **la clase entera**, incluida la asistencia. Primer diagnóstico (equivocado, quedó anotado un rato como D21): que hacía falta una política nueva de confirmar/reliquidar para usuarios con autoridad. **Javier lo corrigió**: el manejo retroactivo ya era correcto (venta retroactiva no bloqueada, liquidación por complemento — regla 16); lo único mal calibrado era el alcance del bloqueo. Medido en el código (regla de calidad 3): por regla 10 el reparto depende de **cuántas clases puso el curso** (calendario menos suspendidas), no de cuánta gente asistió — sumar un alumno a una clase que sigue `dictada` no cambia ese conteo, así que no puede mover la plata ya pagada (`revertirDevengosAbiertos` solo revierte comisiones de liquidaciones **abiertas**, nunca las pagadas). Lo único que sí mueve el conteo es **suspender o reabrir** la clase, o corregir un **descuento al profesor ya pagado** — eso sigue bloqueado. Decisión (Javier, 2026-09-17): permitir tomar/corregir asistencia de una clase congelada (gateado por el permiso `asistencia.editar` que ya exige cargar fechas pasadas), salvo que reabra una sesión suspendida. | **CONSTRUIDA** en dev el 2026-09-17 (`validarFecha` con `operacion:"asistencia"`\|`"suspension"`, `src/app/(privado)/asistencia/acciones.ts`; regla de negocio 16 afinada en `REGLAS.md`). Sin migración. Falta que Javier valide en dev (caso Heels 29/08) y dé el OK del pase. **Nota**: el control 17 de `scripts/control_migracion.sql` puede marcar REVISAR ante una inscripción retroactiva legítima a un período pagado — falso positivo conocido, anotado en `ROADMAP.md` para afinarlo aparte. |
...
- **Visibilidad de datos propios por rol (migración 0043) + fix del mensaje
  ...
  mensaje de rechazo quedaba pegado al cambiar de curso o fecha). **D21 se
  reencuadró el mismo día** (ver §1.b, "Alcance del congelador"): no era una
  política nueva por construir, sino un alcance de la regla 16 a afinar —
  resuelto en un pase posterior. Detalle completo en `docs/ESTADO.md`.
