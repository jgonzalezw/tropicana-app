...
>
> **Faltan tres respuestas para poder arrancar, y las tres son de Javier:**
> 1. **D20 — ¿una sala o varias?** Define el modelo entero. Si no está
>    respondida, no se escribe la primera migración de la agenda.
...
| D20 | **¿La academia tiene UNA sala o varias?** Sin responder, y **define el modelo**: con una sola, la disponibilidad es una línea de tiempo y el choque es "esta hora ya está ocupada"; con varias hace falta una entidad `salas`, cada clase y cada bloqueo dicen en cuál están, y el choque se pregunta **por sala**. Migrar de uno a otro después es rehacer el modelo, no agregarle un campo. **Los datos no lo dicen**: medido el 2026-09-12 en producción, ningún par de cursos se pisa —los que comparten hora no comparten día—, así que la grilla actual es **consistente con una sola sala**, pero eso no prueba que haya una. | **Pendiente de decisión — BLOQUEA el Paso 5** (2026-09-12) | Apareció al construir D6, cuando se pudo medir el solapamiento por primera vez. | **Al arrancar el Paso 5**, antes de escribir la primera migración de la agenda. |
