...
**Progreso Etapa 1:** `0005` ✅ aplicada · `0006` ✅ aplicada · **Profesores** ✅ construido y **probado local** · **Cursos** ✅ construido (componente compartido `EntidadCurso` + pantalla con listado y alta/edición/baja, días como casillas, precio mensual y **tarifas parciales tabla A**; borrado guardado; al existir cursos se activa la pestaña Asignación de Profesores y el filtro de titular por especialidad/estilo; build+lint OK; **pendiente prueba local**). **Alumnos** ✅ construido (componente compartido `EntidadAlumno`: dup por WhatsApp, bloque de tutor si es menor, clave compuesta de menor = WhatsApp tutor + nombre, y tutor que puede ser alumno existente → vincular; canal de captación del catálogo; borrado guardado; build+lint OK; **pendiente prueba local**). **Tomar asistencia** ✅ **construido (núcleo)** — pendiente aplicar `0007` y probar. Migración `0007_etapa1_asistencia` (`sesiones` única por curso+fecha; `asistencias` única por sesión+alumno) validada en Postgres 16 (cadena 0001→0007 limpia; smoke test: upsert de sesión/marcas y consumo de paquete parcial 1/3). Pantalla `/asistencia` (phone-first): elegís curso + fecha → padrón de inscriptos activos (ordenado por apellido); un toque marca presente, otro ausente; "Todos presentes"; contador y pie con leyenda; guardar hace upsert de la sesión y sus marcas (re-editable). Cada **presente consume una clase** del paquete parcial (parcial con 0 restantes deja de aparecer). Titular de la sesión = asignación vigente. Gateada por permiso `asistencia`. Diferido a fase posterior (anotado): tolerancias/reposiciones, clases de prueba en la lista, comisión por clase (0008), rol Profesor/"Confirmar sesión". build+lint+tsc OK.
...
1. **Aplicar migración `0007`** en Supabase (SQL Editor → New query → pegar `supabase/migrations/0007_etapa1_asistencia.sql` → Run). Después probar **Gestión → Tomar asistencia**.
