...
1. **Aplicar migraciones `0008` y `0009`** en Supabase (SQL Editor → New query → pegar cada archivo → Run). `0008` crea el parámetro de ventana retro; `0009` agrega suspensión + corrimiento de fin de ciclo. (`0007` ya aplicada.) Después probar **Gestión → Tomar asistencia** incluyendo el botón de suspender/reabrir.
