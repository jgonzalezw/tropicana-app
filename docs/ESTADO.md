...
**Suspensión de clase — DECISIONES CONFIRMADAS (a construir en `0009`):** reusar el mecanismo de **falta con tolerancia = corrimiento de fin de ciclo** (un solo mecanismo compartido, sin reposición aparte). Hoy ese mecanismo **no existe** (la tolerancia es solo una pastilla informativa): hay que construir el primitivo (tabla traza `corrimientos_ciclo` + mover `vencimiento` de la cuota vigente a la próxima fecha de clase) y engancharle dos disparadores: (1) **falta individual** al marcar ausente a un mensual con tolerancia disponible (cada falta tolerada corre un día; agotada la tolerancia no corre); (2) **suspensión masiva** desde un botón en *Tomar asistencia* que marca la sesión `suspendida` (+`estado`/`motivo` en `sesiones`) y corre el fin de ciclo de **todos** los mensuales del curso. Decisiones: la suspensión **siempre corre y NO gasta** la tolerancia personal (es decisión de la escuela); "fin de ciclo" = **vencimiento de la cuota vigente**; motivo texto libre opcional (catálogo después — **pendiente no olvidar**); permiso `asistencia`. Parciales: no se marca asistencia → no consumen → se difieren solos. Nota: efecto poco visible hasta la pantalla de renovación/estado de cuenta.
...
1. **Aplicar (o re-aplicar) migración `0007`** en Supabase (SQL Editor → New query → pegar `supabase/migrations/0007_etapa1_asistencia.sql` → Run). Es idempotente: re-correrla es seguro y ahora agrega los parámetros `faltas_toleradas` y `mostrar_deuda`. Después probar **Gestión → Tomar asistencia**.
