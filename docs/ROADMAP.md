...
| --- | --- | --- | --- |
| R1 | ~~**Conflicto bloqueo-vs-agendado (C5).**~~ **CERRADO — lado reservas construido en dev el 2026-09-26 (C3, hito H4).** Cuando un bloqueo o un cierre cae sobre clases o reservas ya agendadas, el sistema **junta los conflictos y el humano decide** caso por caso — nunca una cancelación automática silenciosa. **Lado de cursos regulares: en producción desde el 2026-09-16**, y ahora (H4) también cubre el horario reducido, no solo el cierre completo. **Lado de reservas (particular/alquiler): construido en dev.** `calcularImpacto` (`administracion/sala/acciones.ts`) ya mira `reservas_sala` además de cursos; las afectadas se listan, se piden confirmación y pasan a *Suspendida* (la sesión vuelve al saldo). Un bloqueo que choca solo con particular/alquiler ofrece lo mismo antes de rechazar (`crearBloqueoSala`, `sala/acciones.ts`). Detalle en `docs/ESTADO.md`, sección "C3 — H4". | C5 | L |
...
| R22 | ~~**Eliminar una excepción de horario no revierte las suspensiones que generó.**~~ **CERRADO — construido en dev el 2026-09-26 (C3, hito H4).** Encontrado al probar C5 (2026-09-16): guardar el cierre suspende las clases correspondientes, pero borrar la excepción después no las reabría. Ahora `guardarHorarioSala` busca qué quedó ligado a la excepción que se borra (`sesiones.excepcion_id`, `reservas_sala.suspendida_por_excepcion_id`) y pregunta si se revierte antes de borrarla. Para una clase de curso, revertir reabre la sesión; para una reserva, crea una **reserva nueva** en la misma franja (Suspendida sigue siendo un estado final). Extendido simétrico a cancelar un **bloqueo** que había suspendido reservas. Detalle en `docs/ESTADO.md`, sección "C3 — H4". | Paso 5 / C5 | S |

---

## 7. C3 — después de la primera entrega

Lo que las definiciones v2 de C3 (25/09, `docs/relevamientos/2026-09-25-C3-definiciones-v2.md`,
Anexo B) dejan explícitamente para después de la primera entrega. El plan de
la primera entrega (H1–H9) vive en `docs/relevamientos/2026-09-25-C3-plan-construccion.md`.

| # | Qué es | Rebanada | Tamaño |
| --- | --- | --- | --- |
| R28 | **Armado libre de un plan al vender.** La primera implementación son plantillas que se personalizan al vender (H1/H2); la segunda es armar un plan mínimo en el momento de la venta, sin plantilla previa. | C3+ | M |
| R29 | **Servicios especiales con cotización.** Ventas que requieren analizar costos, cantidades y subcontrataciones antes de ofertar: estado previo *cotización* (enviar en formato comercial, confirmar, recotizar, cancelar, declarar perdida), y combinación de elementos de los otros cuatro tipos en una misma venta. | C3+ | L |
| R30 | **Agenda de disponibilidad declarada del profesor.** En C3 alcanza con validar que no choque con sus cursos ni con otras reservas (v2 §8.4); después, el profesor declara en qué horarios está disponible. | C3+ | M |
| R31 | **Costos adicionales y servicios de video.** Recursos humanos adicionales, logística y traslados, equipamiento propio o de proveedores, producción (video, edición de música), terceros subcontratados y los servicios de video (coreografía, evento, institucional). Pertenecen sobre todo a los servicios especiales (R29). | C3+ | L |
